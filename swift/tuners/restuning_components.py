    ...
    def __init__(
        ...
        if (isinstance(tuner_cfg, str) and tuner_cfg == "res_adapter") or \
           (isinstance(tuner_cfg, dict) and "res_adapter" in tuner_cfg):
            ...
            self.tuner = ResAdapter(dim=dim, layer_num=layer_num, depth=depth, zero_init_last=zero_init_last, stage=stage, tuner_cfg=tuner_cfg, **kwargs)
            ...
            self.tuner = ResGroupAdapter(dim=dim, layer_num=layer_num, depth=depth, zero_init_last=zero_init_last, stage=stage, tuner_cfg=tuner_cfg, **kwargs)
            ...
            if 'upsample_out_channels' in kwargs:
                ...
                out_channels = dim
                use_conv = False
            self.tuner = Upsample(channels=dim, use_conv=use_conv, out_channels=out_channels, tuner_cfg=tuner_cfg, **kwargs)
        else:
            self.tuner = Identity()
...
class ResAdapter(nn.Module):
    def __init__(self, dim, layer_num=-1, depth=-1, zero_init_last=False, stage="", tuner_cfg=None, act_layer=nn.GELU, **kwargs):
