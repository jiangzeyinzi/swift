...
class ResTuning:
    ...
    def prepare_model(model: nn.Module, config: ResTuningConfig) -> SwiftOutput:
            ...
            for module_key in module_keys:
                if re.fullmatch(config.root_modules, module_key):
                    ...
                    if isinstance(root_module, (nn.ModuleList, nn.ModuleDict)):
                        logger.warning(f"Type of {type(root_module)} may not be supported because of its customized forward")
            ...
            if (isinstance(config.stem_modules, str) and re.fullmatch(config.stem_modules, module_key)) or \
               (isinstance(config.stem_modules, list) and module_key in config.stem_modules):
                ...
                if isinstance(stem_module, (nn.ModuleList, nn.ModuleDict)):
                    logger.warning(f"Type of {type(stem_module)} may not be supported because of its customized forward")
        ...
        if isinstance(config.stem_modules, list):
            stem_module_ins_list = [stem_module_ins_list[stem_module_ins_index.index(i)] for i in range(len(stem_module_ins_index))]
        ...
        if len(stem_module_ins_list) != 0:
            top_module = model.get_submodule('')
            restuning_module = ResTuningBypassModule(config.dims, depth, config.use_upsample, config.upsample_out_channels, config.zero_init_last, config.tuner_cfg)
        ...
        for module_key in module_keys:
            if re.fullmatch(config.target_modules, module_key):
                ...
                if isinstance(tgt_module, (nn.ModuleList, nn.ModuleDict)):
                    raise Exception(f"Type of {type(tgt_module)} may not be supported because of its customized forward")
    ...
    def __init__(
        ...
        self.bypass_blocks = nn.Sequential(*[
            ResTunerBypassBlock(
                ...
                upsample_out_channels=upsample_out_channels[i] if isinstance(upsample_out_channels, list) else upsample_out_channels,
